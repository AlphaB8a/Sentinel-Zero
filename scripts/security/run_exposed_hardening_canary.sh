...
apply_iptables_rate_limit
start_sentinel "tcp:0.0.0.0:${PHASE2_PORT}" "${PHASE2_LOG}"
...
FW_LEFT="$(sudo iptables -S INPUT | rg --fixed-strings -- ":${PHASE2_PORT}" || true)"

{
  ...
  echo "- Phase2 listen: \`0.0.0.0:${PHASE2_PORT}\` (short-lived)"
  ...
  echo
  echo "## Artifacts"
  echo "- phase1 host log: ${PHASE1_LOG}"
  echo "- phase2 host log: ${PHASE2_LOG}"
  echo "- phase1 client json: ${PHASE1_JSON}"
  echo "- phase2 client json: ${PHASE2_JSON}"
} >"${SUMMARY_MD}"
